...
AudioWrite::AudioWrite(QObject *parent) : QObject(parent)
{
    ...
    //声卡采样格式
     ...
     m_buffer_out = m_audio_out->start();//向buffer 里存放数据，就会播放声音
...
//     // 打印支持的格式
//     qDebug() << "Supported sample rates:" << inputDevice.supportedSampleRates();
//     qDebug() << "Supported channels:" << inputDevice.supportedChannelCounts();
//     qDebug() << "Supported sample sizes:" << inputDevice.supportedSampleSizes();
//     qDebug() << "Supported codecs:" << inputDevice.supportedCodecs();

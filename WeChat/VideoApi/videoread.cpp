...
{
    //qDebug()<<"slot_getVideoFrame Thread:"<<QThread::currentThreadId();
    //从摄像头对象读取数据
    Mat frame;
    if( !cap.read(frame) )
    {
    ...
    if(m_moji!=0)
        m_faceDetect->faceDetectAndDraw(frame,faces);
...
{
    m_timer->start(1000/FRAME_RATE-10); //1000/25=40，每40ms采集一次
    cap.open(0);//打开默认摄像头
    if(!cap.isOpened()){
